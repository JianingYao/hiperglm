...
test_that("return TRUE if hiper_glm with `logit` model and `BFGS` method finds coef estimates match with `glm` one", {
  data <- simulate_data(32, 4, model = "logit", intercept = 1, seed = 1918)
  glm_out <- stats::glm(data$outcome ~ data$design + 0, family = binomial())
  hglm_out <- hiper_glm(data$design, data$outcome, model = "logit", method = "BFGS")
  expect_true(are_all_close(coef(hglm_out), coef(glm_out), abs_tol = 1e-3, rel_tol = 1e-3))
})
...
test_that("return TRUE if analytical and numerical Hessian matrix match in logistic regression", {
  ...
  for (i in 1:n_test) {
    ...
    beta <- rnorm(n_pred)
    eps <- 1e-8
    Hv_approx <- (logit_loglik_gradient(design, outcome, beta + eps * v) - logit_loglik_gradient(design, outcome, beta - eps * v)) / (2 * eps)
    Hv <- logit_hessian(design, beta) %*% v
    grads_are_close <- are_all_close(as.vector(Hv_approx), as.vector(Hv), abs_tol = 1e-3, rel_tol = 1e-3)
  }
  expect_true(grads_are_close)
})

test_that("newton coincides with bfgs and glm outputs coincide on logit model", {
  n_obs <- 32
  n_pred <- 4
